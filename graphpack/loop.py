...
after the first failed, so a run over 2,255 answerable queries came back with a
retrieval score of exactly 0.000. Queries were fixed then; ingest still had its
own `asyncio.run`, which did not matter while ingest and query were always
separate processes and started mattering the moment one command did both.
